...
DELIMITER //
CREATE PROCEDURE sp_buscar_categoria_por_id(IN p_id INT)
BEGIN
    SELECT id_categoria, nombre_categoria
    FROM categorias_activo
...
call sp_buscar_categoria_por_id (1);
...
call sp_borrar_categoria(6);

DROP PROCEDURE IF EXISTS sp_filtrar_categorias_por_nombre;
DELIMITER ##
...
call sp_filtrar_categorias_por_nombre('Laptop');

-- Procedimientos Almacenados para Activos
DELIMITER $$
...
BEGIN
    ...
    FROM 
        activos a
    JOIN 
        categorias_activo c ON a.id_categoria = c.id_categoria
    ORDER BY 
        a.id_activo DESC;
END$$
DELIMITER ;

CALL sp_listar_activos;

DELIMITER //
CREATE PROCEDURE sp_buscar_activo_por_id(IN p_id INT)
BEGIN
    SELECT 
        id_activo,
        id_categoria,
...
CALL sp_buscar_activo_por_id(1);
...
CALL sp_registrar_activo(1, 'SN_TEST_999', 'HP', 'Spectre', '2025-01-01', 1500.00, 'Almacenado');
...
CALL sp_editar_activo(1, 2, 'SN_DELL_001_MOD', 'Dell', 'Monitor U24', '2024-01-10', 450.00, 'En uso');
...
DELIMITER ##
CREATE PROCEDURE sp_filtrar_activos(IN p_valor VARCHAR(100))
BEGIN
    SELECT 
        a.id_activo,
        c.nombre_categoria,
    ...
    WHERE 
        ...
        a.modelo LIKE CONCAT('%', p_valor, '%')
    ORDER BY
        a.id_activo DESC;
END##
DELIMITER ;

CALL sp_filtrar_activos('Dell');

-- Procedimientos Almacenados para licencias_software
...
BEGIN
    SELECT 
        ...
        c.nombre_categoria
    FROM 
        licencias_software l
    JOIN 
        categorias_activo c ON l.id_categoria = c.id_categoria
    ORDER BY 
        l.id_licencia DESC;
END$$
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE sp_buscar_licencia_por_id(IN p_id INT)
BEGIN
    SELECT 
        l.id_licencia,
        l.id_categoria,
...
call sp_buscar_licencia_por_id(1);
...
CALL sp_registrar_licencia('3', 'Azure', 'A365-XXXX-YYYY-ZZZZ', '2025-12-31', 50);
...
BEGIN
    DELETE FROM licencias_software
...
DELIMITER ##
CREATE PROCEDURE sp_filtrar_licencias(IN p_valor VARCHAR(150))
BEGIN
    SELECT 
        l.id_licencia,
        l.nombre_software,
    ...
    JOIN 
        categorias_activo c ON l.id_categoria = c.id_categoria
    WHERE 
        l.nombre_software LIKE CONCAT('%', p_valor, '%') OR
        l.clave_licencia LIKE CONCAT('%', p_valor, '%')
    ORDER BY
        l.nombre_software ASC;
END##
DELIMITER ;
...
END$$
DELIMITER $$;
SELECT * FROM categorias_activo WHERE id_categoria = 4;
